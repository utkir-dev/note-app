package com.example.mynotes.app.models
...
inline fun <T> Flow<ResultData<T>>.onSuccess(crossinline block: suspend (T) -> Unit) = onEach {
    it.onSuccess { block(it.asSuccess.data) }
}


inline fun <T> Flow<ResultData<T>>.onFailure(crossinline block: suspend (MessageData) -> Unit) =
    onEach {
        it.onFail { block(it.asFail.message) }
    }

public inline fun <T> Flow<ResultData<T>>.onFailureText(crossinline action: suspend (String) -> Unit) =
    onEach {
        it.onText { action(message) }
    }

inline fun <T> Flow<ResultData<T>>.onFailureResource(crossinline block: suspend (Int) -> Unit) =
    onEach {
